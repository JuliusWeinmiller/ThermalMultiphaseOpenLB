...
const SimType endTimeEvap = 10010.0*charTime;     // max. simulation time in s, SI unit
...
// == Simulation multiphase flow constants
...
void writeGeometry( olb::SuperLattice2D<SimType, NSDESCRIPTOR>& NSlattice,
                    olb::SuperGeometry2D<SimType>& superGeometry)
{
  olb::SuperVTMwriter2D<SimType> vtmWriter( "evaporatingDroplet" );
  // Writes the geometry, cuboid no. and rank no. as vti file for visualization
  olb::SuperLatticeGeometry2D<SimType, NSDESCRIPTOR> geometry( NSlattice, superGeometry );
  olb::SuperLatticeCuboid2D<SimType, NSDESCRIPTOR> cuboid( NSlattice );
  olb::SuperLatticeRank2D<SimType, NSDESCRIPTOR> rank( NSlattice );
...
// === 1st Step: Initialization ===
  ...
  olb::singleton::directories().setOutputDir( "./evaporatingDroplet/" );
...
// === 3rd Step: Prepare Lattice ===
  ...
  olb::FluidProperties<SimType> fluidProperties( 
                    ...
                    converter.getLatticeThermalConductivity( thermalConductivityVapour*1.5 ),
  ...
  #ifdef ThermalMRT
    thermalMultiphaseCoupling = new olb::CombinedShanChenSemiHybridThermalMRTCouplingGenerator2D<SimType, NSDESCRIPTOR>(
  ...
  #else
    thermalMultiphaseCoupling = new olb::CombinedShanChenSemiHybridThermalCouplingGenerator2D<SimType, NSDESCRIPTOR>(
        0, converter.getLatticeLength(lx0), 
        0, converter.getLatticeLength(ly0),
        G, converter.getLatticeThermalRelaxationFrequency(),
        0, 0, {0,1},
